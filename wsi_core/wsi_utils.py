...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, verbose=1, draw_grid=True):
    ...
    for idx in tqdm(range(total)):
        ...
        if coord[0]/downsamples[0] > wsi_object.level_dim[vis_level][0]:       
            print("Coord X is greater than this level's X dim")
            print("Coord X: {}".format(coord[0]))
            print("Level Dim: {}".format(wsi_object.level_dim[vis_level][0]))
            # exit()
        if coord[1]/downsamples[1] > wsi_object.level_dim[vis_level][1]:
            print("Coord Y is greater than level dim")
            print("Coord Y: {}".format(coord[1]))
            print("Level Dim: {}".format(wsi_object.level_dim[vis_level][1]))
            # exit()

        # print("Patch ID: {}\nCoord: {}".format(patch_id,coord))
        # patch = np.array(wsi_object.wsi.read_region(tuple(coord), vis_level, patch_size).convert("RGB"))
        # print("about to run the wsi.read inside def Draw Map From Coords")
        # print("(series={},XYWH=({}, {}, {}, {}))".format(vis_level,coord[0], coord[1], patch_size[0], patch_size[1]))
        # exit()
        # coord = np.ceil(coord / downsamples).astype(np.int32)
        roi=(
            int(wsi_object.offset[0]+coord[0]), 
            int(wsi_object.offset[1]+coord[1]), 
            int(patch_size[0]*series(vis_level)*downsamples[0]*downsamples[0]), 
            int(patch_size[1]*series(vis_level)*downsamples[1]*downsamples[1])
            )
        patch = wsi_object.wsi.read(zoom=(series(vis_level)), roi=roi)
        coord = np.ceil(coord / downsamples).astype(np.int32)
        canvas_crop_shape = canvas[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0], :3].shape[:2]
        canvas[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0], :3] = patch[:canvas_crop_shape[0], :canvas_crop_shape[1], :]
        if draw_grid:
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1):
    ...
    vis_level = get_best_level_for_downsample(downscale)
    file = h5py.File(hdf5_file_path, 'r')
    dset = file['coords']
    # print("___StitchCoords: dset: {}".format(dset))
    coords = dset[:]
    # print("___StitchCoords: coords: {}".format(coords))
    ...
    print('downscaled WSI size for stiching: {} x {}'.format(w, h))
    ...
    heatmap = np.array(heatmap)
    # print("about to run the DrawMapFromCoords")
...
def SamplePatches(coords_file_path, save_file_path, wsi_object, 
    ...
    for idx in indices:
        coord = coords[idx]
        print("SamplePatches(): patch_size = {}".format(patch_size))
        # print("about to run the wsi.read inside def SamplePatches")
        XYWH=(wsi_object.offset[0]-coord[0], wsi_object.offset[0]-coord[1], patch_size, patch_size)
        patch = wsi_object.wsi.read(zoom=series(patch_level),XYWH=(wsi_object.offset[0]-coord[0], wsi_object.offset[0]-coord[1], patch_size, patch_size))
        if custom_downsample > 1:
