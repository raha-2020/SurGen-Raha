...
# import openslide # Remove openslide to begin czi interface
from PIL import Image
import pdb
import h5py
import math
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, sample_indices, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
...
def get_best_level_for_downsample(downsample):
    if downsample == 0: return 0
    best_level = int(math.log(downsample, 2)) #downsample by 0/1 means use SERIES 0. by 2, use SERIES 1, by 4, use SERIES 2
    return best_level
...
class WholeSlideImage(object):
    def __init__(self, path):
        ...
        Definitions of methods:
            self.name: return the name of the current WSI file. (taken from the end of the given path)

            self.wsi: returns an OpenSlide.open_slide(path?) object which can be used to run OpenSlide queries such as... 
                wsi.get_best_level_for_downsample(downscale):
                    Return the best level for displaying the given downsample.
                    returns a 'pyramid series level' when given a 'factor' to downscale by.
                    i.e. if you want to downscale by 2, use Series=1 instead of Series=0.

                wsi.level_dimensions(k):
                    returns a list of (width, height) tuples, one for each level of the slide. 
                    level_dimensions[k] are the dimensions of level k.

                wsi.level_downsamples(vis_level):
                    returns a list of downsample factors for each level of the slide. 
                    level_downsamples[k] is the downsample factor of level k.

                wsi.read_region(tuple(coord), vis_level, patch_size):
                    returns an RGBA Image containing the contents of the specified region.

                wsi.retrieve_mpp():
                    returns the microns per pixel (mpp) of the slide at Series=0 (Highest resolution).
            
            self.level_downsamples: returns a list of (a,b) tuples, corresponding to the 0(x,y):1(x,y) dimensions. Provides a value of scale. 

            self.level_dim: uses openslide to run level_dimensions(k)
        """

#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        # self.name = os.path.splitext(os.path.basename(path))[0]
        # self.wsi = openslide.open_slide(path) # get OpenSlide.open_slide(path)
        ...
        self.level_dim = self._getLevelDimensions()
        self.level_downsamples = self._assertLevelDownsamples()

        self.offset = self.wsi.total_bounding_rectangle[0], self.wsi.total_bounding_rectangle[1]

        self.get_best_level_for_downsample = 0 # this might be better as a method inside the caller's class.

        # Convert the retrieve_mpp value from meters per pixel to microns per pixel.
        # The original value is provided in meters (e.g., 1.1118506602624068e-07 meters/pixel),
        # which we convert to microns by multiplying by 1e6 (since 1 meter equals 1,000,000 microns).
        # This conversion is necessary because the expected format for mpp in our application is in microns per pixel.
        self.retrieve_mpp = float(self.wsi.metadata["ImageDocument"]["Metadata"]["Scaling"]["Items"]["Distance"][0]["Value"]) * 1e6
    ...
    def _getpylibCZIrwSlide(self):
        # print("inside self.wsi... for path: {}".format(self.path))
        return pyczi.CziReader(self.path)


    def _getLevelDimensions(self):
        # print("Retrieving level dimensions...")
        # start_time = time.time()

        #go through each "series" and note the x and y dims for each inside level_dim
        # print(type(self.wsi))
        ...
        for x in range(10):
            ...
            # print("series(x): {}".format(series(x)))
            #total_rect[2] equals width, total_rect[3] equals height
            dimensions = int(total_rect[2]*series(x)),int(total_rect[3]*series(x))
            # print("dimensions: {}".format(dimensions))
            level_dims[x] = dimensions
            # print("_getLevelDimensions FUNCTION - level_dim: {}".format(level_dim))
        # seg_time_elapsed = time.time() - start_time  
        # print("_getLevelDimensions took {} seconds".format(seg_time_elapsed))

        # print("level_dims[1]: {}".format(level_dims[0]))
        # print("returning level_dims: {}".format(level_dims))
        return level_dims
    ...
    def _assertLevelDownsamples(self):
        ...
        # print("dim_6 = {}".format(self.level_dim[6]))
        
        for dim in self.level_dim:
            estimated_downsample = (dim_0[0]/float(self.level_dim[dim][0]), dim_0[1]/float(self.level_dim[dim][1]))
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        _XYWH =(self.offset[0]-0,                   #x
                self.offset[1]-0,                   #y
                int(self.level_dim[seg_level][0]/series(seg_level)),       #w
                int(self.level_dim[seg_level][1]/series(seg_level)))    #h
        # img = np.array(self.wsi.read(self.path, seg_level, _XYWH))
        img = np.array(self.wsi.read(roi=_XYWH, zoom=series(seg_level)))
        # print("zoom={},roi={}".format(series(seg_level),_XYWH))
        img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.uint8)  # Convert to HSV space
        img_med = cv2.medianBlur(img_hsv[:,:,0].astype(np.uint8), mthresh)  # Apply median blurring
        ...
        # print("inside segmentTissue(). the scale on segmentation level {} is {}".format(seg_level, scale))
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        _XYWH=(self.offset[0]-top_left[0], 
                self.offset[1]-top_left[1], 
                int(region_size[0]/series(vis_level)), 
                int(region_size[1]/series(vis_level)))
        img = np.array(self.wsi.read(zoom=series(vis_level), roi=_XYWH))
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                count+=1
                print("ABOUT TO DO SOMETHING THAT WILL BREAK BECAUSE IT USES BIOFORMATS.")
                patch_PIL = self.wsi.read(series=patch_level, XYWH=(x,y,patch_size,patch_size)).convert('RGB')
                if custom_downsample > 1:
    ...
    def process_contours(self, save_path, target_mpp=1.0, patch_level=0, patch_size=256, step_size=256, storage_format='h5', **kwargs):
        """
        Process contours and save patches.

        Args:
            save_path (str): The path to save the patches.
            target_mpp (float, optional): The target microns per pixel (mpp) resolution. Defaults to 1.0.
            patch_level (int, optional): The level of the slide to extract patches from. Defaults to 0.
            patch_size (int, optional): The size of each patch in pixels. Defaults to 256.
            step_size (int, optional): The step size between patches in pixels. Defaults to 256.
            storage_format (str, optional): The format to save the patches in ('h5' or 'zarr'). Defaults to 'h5'.
            **kwargs: Additional keyword arguments.

        Returns:
            str: The path to the saved patches.

        Raises:
            ValueError: If the attribute dictionary is empty.

        """
        os.makedirs(save_path, exist_ok=True)
        save_path_hdf5 = os.path.join(save_path, f"{self.name}.{storage_format}")
        print("Creating patches for: ", self.name, "...")
        ...
        for idx, cont in tqdm(enumerate(self.contours_tissue), total=n_contours):
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], target_mpp, patch_level, save_path, patch_size, step_size, **kwargs)
            # print(f"attr_dict = {attr_dict}")
            #if attr_dict is empty, throw error
            if not attr_dict:
                raise ValueError("attr_dict is empty")
            
            if asset_dict:  # If we have assets, store them for later saving
                all_asset_dicts.append(asset_dict)
                # if idx == 0:  # Only store attributes once
                all_attr_dicts.append(attr_dict)

        # Concatenate all coordinate arrays
        all_coords = np.concatenate([asset['coords'] for asset in all_asset_dicts], axis=0) if all_asset_dicts else np.array([])

        # print(f"Concatinating a total of {len(all_asset_dicts)} asset dicts to create {len(all_coords)} coordinates")
        #print each all_attr_dicts values
        # for attr_dict in all_attr_dicts:
        #     print(f"attr_dict = {attr_dict}")
        # Choose the first attribute dictionary as they should all be the same for coords
        if all_attr_dicts:
            attr_dict = all_attr_dicts[0]
        else:
            raise ValueError("all_attr_dicts is empty")

        # Now save all coordinates at once
        if storage_format == 'h5':
            # For HDF5, you'd call your existing save_hdf5 function
            save_hdf5_coords(
                output_path=save_path_hdf5, 
                asset_dict={'coords': all_coords}, 
                attr_dict=attr_dict, mode='w', 
                backend=storage_format)
        elif storage_format == 'zarr':
            # For Zarr, ensure your save_hdf5 can handle zarr format
            save_hdf5_coords(output_path=save_path_hdf5, asset_dict={'coords': all_coords}, attr_dict=attr_dict , mode='w', backend='zarr')

        return save_path_hdf5  # Or the Zarr path if using Zarr

    def generate_all_patch_coords(self, target_mpp=1.0, patch_level=0, patch_size=256, step_size=256):
        """
        Generate all patch coordinates for the whole slide image based on target_mpp and patch_level.
        Adjusts patch and step sizes based on the target microns per pixel (target_mpp).
        """

        # Adjust patch and step sizes based on the target_mpp
        adjusted_patch_size = int(patch_size * target_mpp / self.retrieve_mpp)
        adjusted_step_size = int(step_size * target_mpp / self.retrieve_mpp)

        # Get image dimensions at the desired level
        img_w, img_h = self.level_dim[patch_level]

        # Initialize a list to hold all patch start coordinates
        all_patch_coords = []

        # Generate coordinates for each patch
        for y in range(0, img_h - adjusted_patch_size + 1, adjusted_step_size):
            for x in range(0, img_w - adjusted_patch_size + 1, adjusted_step_size):
                all_patch_coords.append((x, y))
    ...
    def process_contour(self, cont, contour_holes, target_mpp, patch_level, save_path, patch_size = 256, step_size = 256,
        contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None):

        #patch_size must be adjusted to operate with the target_mpp. This can be calculated by multiplying the patchs size by the target_mpp and dividing by the current mpp.

        # print debug, target_mpp and it's type, as well as patch_size and it's type
        # print("DEBUG: target_mpp: {}, type(target_mpp): {}".format(target_mpp, type(target_mpp)))
        # print("DEBUG: patch_size: {}, type(patch_size): {}".format(patch_size, type(patch_size)))
        # print("DEBUG: step_size: {}, type(step_size): {}".format(step_size, type(step_size)))
        # print("DEBUG: patch_level: {}, type(patch_level): {}".format(patch_level, type(patch_level)))
        # print("DEBUG: self.retrieve_mpp: {}, type(self.retrieve_mpp): {}".format(self.retrieve_mpp, type(self.retrieve_mpp)))
        # print("DEBUG: self.level_downsamples[patch_level]: {}, type(self.level_downsamples[patch_level]): {}".format(self.level_downsamples[patch_level], type(self.level_downsamples[patch_level])))
        # print("DEBUG: self.level_dim[patch_level]: {}, type(self.level_dim[patch_level]): {}".format(self.level_dim[patch_level], type(self.level_dim[patch_level])))

        patch_size = int(patch_size * target_mpp / self.retrieve_mpp) # round pixel down to nearest integer
        # print("patch_size: {}".format(patch_size))
        # step_size must match
        step_size = int(step_size * target_mpp / self.retrieve_mpp) # round pixel down to nearest integer
        # print("step_size: {}".format(step_size))

         

        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
        # print("working on file: {}".format(self.name))
        patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        ref_patch_size = (patch_size*patch_downsample[0], patch_size*patch_downsample[1])
        ...
        if len(results)>1:
            # print("DEBUG: results.shape: {}".format(results.shape))
            ...
            # print("DEBUG: RETURNING EMPTY BRACES BECAUSE len(results)>1: results.shape: {}".format(results.shape))
            attr = {'patch_size' :            patch_size, # To be considered...
                    'patch_level' :           patch_level,
                    'downsample':             self.level_downsamples[patch_level],
                    'downsampled_level_dim' : tuple(np.array(self.level_dim[patch_level])),
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if not blank_canvas:
            ...
            print("ABOUT TO DO SOMETHING THAT WILL BREAK BECAUSE IT USES BIOFORMATS.")
            img = np.array(self.wsi.read(series=vis_level, XYWH=(top_left[0],top_left[1],region_size[0],region_size[1])).convert("RGB"))
        else:
            # use blank canvas
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    pt = (x_start, y_start)
                    print("ABOUT TO DO SOMETHING THAT WILL BREAK BECAUSE IT USES BIOFORMATS.")
                    canvas = np.array(self.wsi.read(series=vis_level, XYWH=(pt[0],pt[1],blend_block_size[0],blend_block_size[1])).convert("RGB"))
                else:
