"""
Stage 2: 5-year survival using XGBoost on Stage-1 features (SR386).

Same inputs and CSV layout as survival_stage2_main.py (emb_* + prob_* or ablations).
Uses XGBClassifier with optional early stopping on the validation set.
Class balance uses scale_pos_weight = n_negative / n_positive on train.

Requires: pip install xgboost

Run from reproducibility/:
  python survival_stage2_xgboost.py \\
...
Optional (Stage 1 test AUROCs + JSON):
  ...
  --out_metrics_json exports/stage2_xgb_metrics.json
"""
...
def _scale_pos_weight(y: np.ndarray) -> float:
    """Match balanced positive weight: n_neg / n_pos for binary y in {0,1}."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Stage 2 survival (5Y_SUR) with XGBoost")
    ...
    print("Stage 2 (XGBoost) feature columns (%d): %s%s" % (len(feat_cols), preview, suf))
    for c in feat_cols:
    ...
    print("XGBoost scale_pos_weight (n_neg/n_pos on train): %.4f" % spw)
    ...
    if best_iter is not None:
        print(f"\nXGBoost best_iteration (early stopping): {best_iter}")
    ...
    if args.out_metrics_json:
        ...
        out = {
            ...
            "xgb_best_iteration": int(best_iter) if best_iter is not None else None,
